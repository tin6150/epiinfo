...
vfdb vs resfind output are very similar, same structure, just different gene names listed.

the summary output, despite changing number of cols depending on dataset, 
end up being easier to parse than the combined file version, 
as there are less "distractions" to parse.
...
#++ these aren't handled correctly by this code yet (it doesn't parse correctly and get converted to NA or some such, another Rmd has getFirstNum() fn to parse them)  
