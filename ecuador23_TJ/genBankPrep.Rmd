...
isolate_list = isolate_list_csv %>% select(`_Isolate`) %>%
  rename( isolate = `_Isolate` )


str( isolate_list )

dummy = isolate_list %>% mutate( filename = str_match( isolate, "(S)" ) )
  
dummy = isolate_list %>% mutate( filename = str_match( isolate, "(S)" ) [,2])   # 
head(dummy)

  
table4genBankSubmission = isolate_list %>%
  #                                       2                 3                      4                  5                      6              7                          
  #mutate( Snum     = str_match( isolate, "(S[:digit:]+)[\\_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX[:digit:][\\-_]L([:alnum:]+)" )[,2]) #%>% 
  ...
  mutate( filename = str_match( isolate, "(S[:digit:]+[\\_]CKDN[:digit:]+[\\-][:alnum:]+[\\-_]H[:alnum:]+DSX[:digit:][\\-_]L[:alnum:]+.fasta)/assembly.fasta" )[,2] )
   #mutate( filename = str_match( isolate, "bla" ) )
    ###mutate( Hnum            = str_match( isolate, "(A[:alnum:]+)[\\-_](CKDN22[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX5[\\-_]L([:alnum:]+)\\_(vfdb|ecoli_vf).txt"  )[,5] )  #%>%



head( table4genBankSubmission )
