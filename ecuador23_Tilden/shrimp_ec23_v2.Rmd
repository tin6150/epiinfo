...
# checking procedure...   get sample and their genes
...
View( amrCountPerSample )  # 26 rows
print( n=26, amrCountPerSample )                       
#########
...
SampleWith2plusArg = 
  resfinder_labeled_noDup %>% 
  group_by( sample_name )  %>% 
  summarize( numOfAmrclass = length(amr_type)  ) # %>%   # 106 without the filter
  filter( numOfAmrclass > 1 )             # 42 with filter

nrow( SampleWith2plusArg )
View( SampleWith2plusArg )


# it is same result, summarize n() is tally, could have used other fn with summarize, eg mean
SampleWith2plusArg_TallyMethod = 
  resfinder_labeled_noDup %>% 
  group_by( amr_type, sample_name ) %>% 
  tally()  %>%                # 106 w/o filter 
  filter( n > 1 )             # 42 with filter
nrow( SampleWith2plusArg_TallyMethod )   
