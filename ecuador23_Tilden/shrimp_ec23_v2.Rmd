...
MOBrecon_mge_result_tsv = MOBrecon_mge_result_tsv %>%
  #                                                2                 3                    4                  5                      6              7=DB specific
  ...
  mutate( sampleIdJoinKey    = str_c( sample_name, "_H", Hnum, "_L", Lnum,   sep="" ) )
  
resfinderOnPlasmid_lng_tsv = resfinderOnPlasmid_lng_tsv %>%
  #                                                2                 3                    4                  5                      6              7                8
...
plasmid_in_mge_result = MOBrecon_mge_result_tsv %>% filter( molecule_type == "plasmid" ) %>% select( sampleIdJoinKey, primary_cluster_id, secondary_cluster_id ) %>% unique() 
  ##%>%   pivot_longer( cols=c( primary_cluster_id, secondary_cluster_id ), names_to="cluster_type", values_to="cluster_id" )
                ...
                View( plasmid_in_mge_result )  # 29 rows

plasmid_in_mge_result %>% select( sampleIdJoinKey ) %>% unique()   # 21 samples

mgeResult_arg = left_join( plasmid_in_mge_result, resfinderOnPlasmid_lng_tsv, by="sampleIdJoinKey" )
