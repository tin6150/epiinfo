...
mlst_resfind_vfdb_df_condensed = mlst_resfind_vfdb_df %>%
...
ggplot( data = resAmrCountByCategorySanOther, 
        #mapping=aes( x=amr_type, y=percentage, fill=source ) 
        mapping=aes( x=forcats::fct_reorder(amr_type,desc(percentage)), y=percentage, fill=source ) 
      ) +
...
```
  ggplot( data=resAmrCountByCategory, 
               aes( x=amr_type, y=percentage) ) +
               geom_bar( aes(col=source), stat="identity", position="dodge" ) 
