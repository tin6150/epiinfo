...
resultV2_raw_checkDup = resultV2_raw %>%  # resultV2 has "I" sample removed as it has NA due to lack of fasta
  select( Filename, source, Sampling_Loc, site, date, PubMLST_scheme_name, sample_name.x, SeqType, RESIST_NUM_FOUND, VFDB_NUM_FOUND, ECVF_NUM_FOUND,  Hnum.x, Lnum.x )


print( "check these as duplicates?  #++" )                    
...
resfinder_labeled <- resfinder_labeled %>%
  mutate(amr_type = case_when(
    ...
    # Macrolides
    str_detect(gene, "mdf") |    # TR Fig 4 include mdf.A here, 28 rows.  
...
fig4 = resAmrCountByGene %>% ggplot( aes(x = factor(AbrGene), y = AbrGeneCount))

fig4 + facet_grid( amr_type ~ . , scales="free_y", space="free") + 
geom_bar(stat="identity",aes(fill=factor(resAmrCountByGene$source)),position="dodge")  +
  theme_bw()+theme(axis.text.x=element_text(angle=60,hjust=1))+coord_flip()    # hmm... the theme removed the space of otherwise empty bars!  
# larger panel for those with more bars!

```

```{r highHope_withFactor}

# https://stackoverflow.com/questions/68508929/how-to-remove-extra-column-in-facet-wrap-plot-with-ggplot2
# this is what's desired for Shrimp paper Fig 4

#Name = c(factor(Amr_))

resAmrCountByGene %>% ggplot(aes(AbrGene, AbrGeneCount))+
  geom_bar(stat = "identity")+
  coord_cartesian(ylim = c(0, 30)) +
  facet_grid(~factor(amr_type), scales="free") 
# + coord_flip()
# very promising till flipped the coordinate!
  
  #facet_wrap(~factor(amr_type), scales="free")


```


```{r highHope_butNotYet}

# https://stackoverflow.com/questions/68508929/how-to-remove-extra-column-in-facet-wrap-plot-with-ggplot2
# this is what's desired for Shrimp paper Fig 4
resAmrCountByGene %>% ggplot(aes(amr_type, AbrGeneCount))+
  geom_bar(stat = "identity")+
  coord_cartesian(ylim = c(0, 25)) +
  facet_wrap(~factor(AbrGene), scales="free")


```

```{r barGraphForFig4_geomPoint_didntWorkOut }

# https://stackoverflow.com/questions/10183292/how-can-i-remove-empty-factors-from-ggplot2-facets

# fig4 = 
ggplot( data = resAmrCountByGene,
        mapping=aes( y=amr_type, x= AbrGeneCount, xmin=0, xmax=25)
        ) + 
  geom_point() +
  geom_errorbarh( height=0 ) + 
  facet_grid( AbrGene ~ ., scales="free", space="free")

```


```{r barGraphForFig4_another_noGood }

# fig4 = 
ggplot( data = resAmrCountByGene,
        mapping=aes( x=amr_type, y= AbrGeneCount, fill=`source` )
        #mapping=aes( x=AbrGene, y= AbrGeneCount, fill=amr_type ) 
        ) + 
  geom_col(  position="stack" ) + 
  facet_grid( . ~ amr_type, drop=T, space="free" ) #  +   # switch="both"  space="free_x"
coord_flip()
...
# fig4 = 
ggplot( data = resAmrCountByGene,
        mapping=aes( x=AbrGene, y= AbrGeneCount, fill=`source` )
        #mapping=aes( x=AbrGene, y= AbrGeneCount, fill=amr_type ) 
        ) + 
  geom_bar( stat="identity", position="stack" ) + 
  facet_grid( . ~ amr_type, switch="both" ) +
  coord_flip()

```

```{r barGraphForFig4_noGrping, eval=F }
...
```{r usefulDF, eval=F }
View( resfinder_labeled )      # long format
