...
helper fn to convert df to setList needed for the venn diagram
 generation
 
```{r df4ListOfSets} 

df_to_set_list_claude <- function(df) {
  set_list <- list()
  ...
  for (i in 1:nrow(df)) {
    ...
    for (j in 1:ncol(df)) {
      ...
      # Check if the cell is not empty or NA
      if (!is.na(cell_value) && cell_value != "") {
        # If the cell contains a comma-separated list, split it
        if (grepl(",", cell_value)) {
          set_values <- trimws(unlist(strsplit(cell_value, ",")))
        } else {
          set_values <- cell_value
        }
        row_sets[[col_name]] <- set_values
      }
    }
    set_list[[i]] <- row_sets
  }
...
# Function to convert a row to a "set" (unique vector)
row_to_set <- function(dataframe, row_index) {
  # Extract the row
  row <- dataframe[row_index, ]
  
  # Convert to vector and remove duplicates
  set <- unique(unlist(row))
...
# result <- df_to_set_list(df)

# specieAndArg = resGeneBySpecieCount %>%
specieAndArg_Wide = resGeneBySpecieCount %>%
  pivot_wider( names_from=gene, values_from=numResGene ) 
...
# SET1 <- row_to_set(specieAndArg, 1)
# SET2 <- row_to_set(specieAndArg, 2)

bact1 = row_to_set( resAmrCountByGene_wide, 1 )
bact2 = row_to_set( resAmrCountByGene_wide, 2 )

ListOfSet = list( Bact1 = bact1, Bact2 = bact2 )


# Name = SET1$[1] Specie_Name

ListOfSet = list( Set1 = SET1, Set2 = SET2 )
row.names( ListOfSet ) = list( "Set1", "Set2" )


str( ListOfSet )

##XX specieAndArg4venn <- df_to_set_list( specieAndArg )

specieAndArg4venn <- df_to_set_list( resGeneBySpecieCount )
str( specieAndArg4venn )
...
##xx  x = specieAndArg4venn  ## Sn50   not, not the List of Set needed.
x = ListOfSet
