...
```{r statForManuscriptFig4, eval=F}


# number of unique ARG found
as.data.frame( resAmrCountByGene ) %>% select ( gene )  %>% 
  unique() %>% 
  count()
# 61
...
sum( geneCount_by_amrClass$counter ) # 71.  so counter must be double counting by source or something
