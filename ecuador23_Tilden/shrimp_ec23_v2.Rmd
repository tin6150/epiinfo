...
mge_location = MOBrecon_mge_result_df %>% 
  ...
  select( sample_id, sample_name, primary_cluster_id, secondary_cluster_id, contig_start, contig_end, length )
...
mgeLocPlusARG = left_join( mge_location, resfinder_location,
                           relationship = "many-to-many",
                           by=c("sample_name"="sample_name") )  # M:M :-\   524 rows

mgeLocPlusARG = mgeLocPlusARG %>%
  mutate( goodStart = ifelse( ( (contig_start <= START)  ), TRUE, FALSE ), 
          goodEnd   = ifelse( ( (contig_end   >= END)    ), TRUE, FALSE ),
          InsideRange = ifelse (goodStart & goodEnd, TRUE, FALSE)
          ) 
  

  #mutate( goodStart = ifelse( ( (contig_start <= START) & (END <= contig_end) ), TRUE, FALSE ) )

  #mutate( argInsideMGE = if_else( (contig_start <= START & END <= contig_end ), TRUE, FALSE ) )  # if_else should be vectorized, but still wrong!
  #?? mutate( argInsideMGE = ifelse( (contig_start <= START & END <= contig_end ), TRUE, FALSE ) )  # this AI code doesnt work.  cuz ifelse() is not vectorized

mgeLocPlusARG$goodStart = ifelse( ( (as.numeric(mgeLocPlusARG$contig_start) <= mgeLocPlusARG$START)  ), TRUE, FALSE )
mgeLocPlusARG$goodEnd   = ifelse( ( (as.numeric(mgeLocPlusARG$contig_end)   >= mgeLocPlusARG$END)    ), TRUE, FALSE )
mgeLocPlusARG$InsideRange = ifelse (mgeLocPlusARG$goodStart & mgeLocPlusARG$goodEnd, TRUE, FALSE)


argInsideMGE = mgeLocPlusARG %>% 
  select( - file_name.x, - length ) # %>%
  filter( argInsideMGE == TRUE )  # 64 rows

View( mgeLocPlusARG )  # 524 rows
View( argInsideMGE )  # 64 rows
