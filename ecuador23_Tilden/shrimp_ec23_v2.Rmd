...
df4venn = resGeneBySpecie %>% group_by( Specie_Name )  %>% reframe( PRODUCT )

ResGene_Ecoli      = list( df4venn %>% filter( Specie_Name == "Escherichia coli"      ) %>% pull( PRODUCT ) ); 
ResGene_Ecoli
...
ResGene_Klebsiella = list( df4venn %>% filter( Specie_Name == "Klebsiella pneumoniae" ) %>% pull( PRODUCT ) ); 
ResGene_Klebsiella = noquote(ResGene_Klebsiella)

# gua_abricate_analy.Rmd line 2324 use these noquote() thing...   but needed to save ?
