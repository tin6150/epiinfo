...
2025.0118: v3, slight update, only to get specie info (E.coli) for use in Integron project, create new tsv. changes are around:
  Line  50: 		specieName = read_tsv( "metadata/specieName.tsv" )
  Line 314:     sample_metadata = ResMlstCombTable50 %>%
  Line 329:   	write_tsv( sample_metadata, "metadata/sample_metadata.tsv" ) 

sample_metadata_withDupInfo.tsv (v3) is created by hand, coping dup info for isolate AF, AG, AT, P, Q.
...
###
###  read MOB-typer result 
### 
# https://github.com/phac-nml/mob-suite?tab=readme-ov-file#mge-detection
# predicted_mobility col reports conjugative vs non-mobilizable, but it is not specific to which gene.  Need ARG specific info...
MOBtyper_result_file = sprintf( "%s/%s", DATADIR, "combined.MOBty.report.tsv")
MOBtyper_result_tsv = read_tsv( MOBtyper_result_file, col_types = list( .default=col_character() ) )
#View( MOBtyper_result_tsv )


### read MOB_recon mge result
...
## there are position of the MGE...
## see analysis procedrue below `mge_plasmid_check` 

MOBrecon_mge_result_file = sprintf( "%s/%s", DATADIR, "MOB_recon_combined.mge.report.tsv")
MOBrecon_mge_result_tsv = read_tsv( MOBrecon_mge_result_file, col_types = list( .default=col_character() ) )
#View( MOBrecon_mge_result_tsv )

resfinderOnPlasmid_lng_file = sprintf( "%s/%s", DATADIR, "abricate_resfinder_allPlasmids.tsv")
resfinderOnPlasmid_lng_tsv = read_tsv( resfinderOnPlasmid_lng_file, col_types = list( .default=col_character() ) ) %>%
  rename( file_name = '#FILE' ) 
...
# v2, computer's CACHE/ dir ## ResMlstCombTableTsv50  = sprintf( "%s/50_Sample\ Info\ Added_mlst.all\ -\ mlst.all.tsv", CACHEDIR )   

ResMlstCombTableTsv50  = sprintf( "%s/Sn50Copy_of_Sample_Info_Added_mlst.all.xlsx", CACHEDIR )    # v3, on wombat

ResMlstCombTable50     = readxl::read_xlsx( ResMlstCombTableTsv50 ) %>%    # edited header 
  rename( file_name = 'Filename' ) %>%
  rename( SDE       = 'SAMPLING_LOCATION', 
          date      = 'DATE_SAMPLED', 
          site      = 'FARM_TYPE', 
          source    = 'SAMPLE_TYPE',
          )  # these rename cuz code ~Line 314 for "sample_metadata" was coded to use such name # v3 2025.0118
...
sample_metadata = ResMlstCombTable50 %>%
  dplyr::select( date, site, source, SDE, file_name, sample_name, Order, PubMLST_scheme_name, SeqType, key, Hnum, Lnum, ctrl ) %>%  # v2
  #  dplyr::select( date, site, source, SDE, file_name, sample_name, key, Hnum, Lnum, ctrl ) %>%  # v2
...
#// no 
#//  "D_|E_|L_|AC_|AE_|AK_|AL_|AN_|AW_|AZ_|BC_|BD_"
#//   n  y  n  n   n   n   n   n   n   n   n   n

#// re cross check...
resfinder_labeled_noDup %>% select(sample_name) %>% unique()   #// 32 samples only
...
```{r plotForFig2, eval=F }
#### ++ TBD ... skipping for now cuz ggtext not loading 
...
```{r esblGeneCount__NOT_useful, eval=F }

resAmrCountByGene %>% 
    filter( grepl("blaCTX-M|blaTEM|blaSHV|blaOXA", gene) )

# no, above is just AMR gene list, no sample info.
...
```{r fig3, eval=F}
# see next chunk below, where % label is "repel" 

#  weasel p_load(ggtext) works, but seems not on zyzyxia3
...
#library(pak)
#pak::pak( "ggtext" ). # seems like failing on zyzyxia3 as well.  on a dep png
#pak( "ggtext" ) # supposed to be fast, but just loop forever on weasle!
...
p_load(ggrepel)
p_load(ggtext)
# then replace geom_text with geom_text_repel

# https://r-charts.com/part-whole/pie-chart-labels-outside-ggplot2/

df4fig3alt = bacteriaDistributionWithSpecieName %>%
  mutate(csum = rev(cumsum(rev(speciePercent))), 
         pos = speciePercent/2 + lead(csum, 1),
         pos = if_else(is.na(pos), speciePercent/2, pos))

#bacteriaDistributionWithSpecieName %>% 
df4fig3alt %>%  
  ggplot( aes( x="", fill=fct_inorder(specie_legend_label), y=speciePercent )) + 
  geom_col() + 
  geom_col(color = "black") +
  coord_polar( theta="y" ) + 
  #scale_fill_brewer(palette = "Pastel1") +
  geom_label_repel(data = df4fig3alt,
                   aes(y = pos, 
                       label = percent_label
                       ),
...
```{r exportPieDataForExternalDrawer}

df4pie = bacteriaDistributionWithSpecieName %>%
  select( Specie_Name, speciePercent, specie_legend_label )

df4pie

writexl::write_xlsx( df4pie, "TMP/figPie.xlsx" )
...
# use resGeneBySpecie[Count] to spot check venn diagram ++
resGeneBySpecie4vennCheck = resGeneBySpecie %>%
  select( file_name.x, sample_name.x, gene, PRODUCT, Specie_Name )
#View( resGeneBySpecie4vennCheck )
...
#### ++ use this as basis to create venn diagram, cross check with venn text below.
...
```{r ListOfSets4venn, eval=F} 


specieAndArg_Wide = resGeneBySpecieCount %>%
  select( Specie_Name, gene ) %>% 
  pivot_wider( names_from=Specie_Name, values_from=gene ) 

View( specieAndArg_Wide )

ListOfSet = list( Bact1 = bact1, Bact2 = bact2 )

Aeromonas_spp     = row_to_set( specieAndArg_Wide, 1 )
`Citrobacter freundii` = specieAndArg_Wide[[2]] # %>% select( `specieAndArg_Wide` )
`Enterobacter cloacae` = specieAndArg_Wide[[3]] # %>% select( `specieAndArg_Wide` )
`Escherichia coli`     = specieAndArg_Wide[[4]] # %>% select( `specieAndArg_Wide` )
`Klebsiella pneumoniae` = specieAndArg_Wide[[5]] # %>% select( `specieAndArg_Wide` )
`Pseudomonas aeruginosa` = specieAndArg_Wide[[6]] # %>% select( `specieAndArg_Wide` )
`Pseudomonas putida`     = specieAndArg_Wide[[7]] # %>% select( `specieAndArg_Wide` )

ListOfBacteria = list( Aeromonas_spp = Aeromonas_spp, 
...
```{r veryLongVerboseOutputForTest eval=F}
# Print the result    ## useful to kinda see.
for (i in 1:length(specieAndArg4venn)) {
  cat("Row", i, ":\n")
  print(result[[i]])
  cat("\n")
}
...
```{r textVenn_testData, eval=F}

# this seems to provide all set region combination.
# cross check with the heat map table generated above.
...
p_load(ggVennDiagram)
#library(ggVennDiagram)

## Sn50   from above chunk
x = ListOfBacteria


# Function to get all specific intersections
get_specific_intersections <- function(sets) {
  ...
  all_combs <- unlist(lapply(1:n, function(i) combn(set_names, i, simplify = FALSE)), recursive = FALSE)
  
  for (comb in all_combs) {
    intersection <- Reduce(intersect, sets[comb])
    other_sets <- setdiff(set_names, comb)
    if (length(other_sets) > 0) {
      intersection <- setdiff(intersection, Reduce(union, sets[other_sets]))
    }
...
# Get and print all specific intersections
intersections <- get_specific_intersections(x)

# Sort intersections by the number of sets involved, then alphabetically
sorted_intersections <- names(intersections)[order(sapply(strsplit(names(intersections), "&"), length), names(intersections))]

for (name in sorted_intersections) {
  cat("Elements in", name, "but not in other sets:\n")
  cat(paste(intersections[[name]], collapse = ", "), "\n\n")
}

# Create the Venn diagram #// no use, numerical % even for 2 sets!
...
above generated this output
cross check with heat map table
at the same time paste to venn diagram (Lucid Chart)


**output is wrong**  code isn't doing what's needed for venn diagram.
per df4venn_wide (DF, _matrix version also exist)
* mdfA is in e.coli and P.aeruginosa (TR had it in intersection of 4 bacteria)
* oqxA is in 3 bacteria, E.coli, E.choacae, Klepsiella, but this output has that intersetion as empty., listing oqxA_1 under E.coli only.


Elements in Aeromonas_spp but not in other sets:  # top left red
ampH_1, ampH_2, blaCMY-8b_1, cphA1_1, cphA2_6, blaCMY-98_1, qnrB69_1, blaCMH-3_1, fosA_7, oqxA_1, oqxB_1, aadA5_1, aph(3'')-Ib_2, aph(3'')-Ib_5, aph(3')-IIa_2, aph(6)-Id_1, blaACT-6_1, blaCMY-2_1, blaCTX-M-130_1, blaCTX-M-15_1, blaCTX-M-27_1, blaCTX-M-55_1, blaCTX-M-8_1, blaTEM-141_1, blaTEM-1B_1, dfrA14_5, dfrA17_1, floR_2, fosA_1, mdf(A)_1, mph(A)_2, qnrS1_1, sul1_5, sul2_2, tet(A)_6, tet(B)_1, tet(B)_2, aac(3)-IIa_1, aac(6')-Ib-cr_1, aadA2_1, ant(3'')-Ia_1, aph(3')-Ia_1, blaOXA-10_1, blaOXA-1_1, blaSHV-106_1, blaSHV-145_1, blaSHV-27_1, cmlA1_1, dfrA12_8, dfrA7_5, fosA6_1, fosA_6, qnrB1_1, aph(3')-IIb_2, blaOXA-485_1, blaOXA-486_1, blaPAO_2, blaPAO_4, catB7_1, fosA_4, blaSED1_1  # copied

Elements in Citrobacter freundii but not in other sets:  # new bottom left
c("blaCMY-98_1", "qnrB69_1")                             # copied    

Elements in Enterobacter cloacae but not in other sets: # bottom right
c("blaCMH-3_1", "fosA_7", "oqxA_1", "oqxB_1")          # copied

Elements in Escherichia coli but not in other sets:   (bottom center)
c("aadA5_1", "aph(3'')-Ib_2", "aph(3'')-Ib_5", "aph(3')-IIa_2", "aph(6)-Id_1", "blaACT-6_1", "blaCMY-2_1", "blaCTX-M-130_1", "blaCTX-M-15_1", "blaCTX-M-27_1", "blaCTX-M-55_1", "blaCTX-M-8_1", "blaTEM-141_1", "blaTEM-1B_1", "dfrA14_5", "dfrA17_1", "floR_2", "fosA_1", "mdf(A)_1", "mph(A)_2", "oqxA_1", "oqxB_1", "qnrS1_1", "sul1_5", "sul2_2", "tet(A)_6", "tet(B)_1", "tet(B)_2") 
# copied

Elements in Klebsiella pneumoniae but not in other sets:   # top right
c("aac(3)-IIa_1", "aac(6')-Ib-cr_1", "aadA2_1", "ant(3'')-Ia_1", "aph(3'')-Ib_5", "aph(3')-Ia_1", "aph(6)-Id_1", "blaCTX-M-15_1", "blaOXA-10_1", "blaOXA-1_1", "blaSHV-106_1", "blaSHV-145_1", "blaSHV-27_1", "blaTEM-1B_1", "cmlA1_1", "dfrA12_8", "dfrA14_5", "dfrA7_5", "fosA6_1", "fosA_6", "mph(A)_2", "oqxA_1", "oqxB_1", "qnrB1_1", "sul1_5", "sul2_2", "tet(A)_6") 
# copied

Elements in Pseudomonas putida but not in other sets:  # top left green
c("aph(3')-IIb_2", "blaOXA-485_1", "blaOXA-486_1", "blaPAO_2", "blaPAO_4", "catB7_1", "fosA_4")   # copied

Elements in Aeromonas_spp & Pseudomonas aeruginosa but not in other sets:
NA 


# the above venn text does not have Set 1 & Set 2 ... *sigh*  attempt to recover code below, but more effort than likely is worth.

just use the heap map matrix... 

# cross check venn text with heatMap matrix

```{r crossCheck_vennText_withHeatMap }

View( df4venn_wide_matrix )   ## ++ use this as basis to create venn diagram, cross check with venn text above.  also check to see how many genes have overlap. ++
View( df4venn_wide )
#//write_tsv( as.data.frame(df4venn_wide), "TMP/df4venn_wide.TSV" )
#writexl::write_xlsx( as.data.frame(df4venn_wide), "TMP/df4venn_wide.XLSX" )  ## this is a sparse sheet, good enough to xfer to venn diagram!  annotated it, moved to iCloud sn127


# these are code from above, just as reference to search for it back.  around line 1351
# df4venn_wide_matrix = df4venn_wide_matrix %>% replace(is.na(.), 0)
# rownames( df4venn_wide_matrix ) = df4venn_wide$gene 
# fig5alt = heatmap( df4venn_wide_matrix )     # this heatmap figure does work ++
heatmap( df4venn_wide_matrix )     # this heatmap figure does work ++   but there aren't 63 genes here


fig5alt    # this does not, just print some numbers...  
...
Hmm... still need the other intersections... 

```{r the_other_regions, eval=F}

# fished this chunk back from commit a7546e5

# Function to get all specific intersections
get_specific_intersections_ver_a7546e5 <- function(sets) {
  set_names <- names(sets)
  n <- length(sets)
...
# Get and print all specific intersections
intersections_a7546e5 <- get_specific_intersections_ver_a7546e5(x)

# Sort intersections by the number of sets involved
sorted_intersections_a7546e5 <- names(intersections_a7546e5)[order(sapply(strsplit(names(intersections_a7546e5), "&"), length))]

for (name in sorted_intersections_a7546e5) {
  cat("Elements in", name, "but not in other sets:\n")
  cat(paste(intersections_a7546e5[[name]], collapse = ", "), "\n\n")
}
...
```{r textVenn_setAND_should_be_in_this_version}

1==0

```

```{r yetAnotherOlderVersion, eval=F}
...
no set AND in this output.


Region: Aeromonas_spp 
Elements: ampH_1, ampH_2, blaCMY-8b_1, cphA1_1, cphA2_6, blaCMY-98_1, qnrB69_1, blaCMH-3_1, fosA_7, oqxA_1, oqxB_1, aadA5_1, aph(3'')-Ib_2, aph(3'')-Ib_5, aph(3')-IIa_2, aph(6)-Id_1, blaACT-6_1, blaCMY-2_1, blaCTX-M-130_1, blaCTX-M-15_1, blaCTX-M-27_1, blaCTX-M-55_1, blaCTX-M-8_1, blaTEM-141_1, blaTEM-1B_1, dfrA14_5, dfrA17_1, floR_2, fosA_1, mdf(A)_1, mph(A)_2, qnrS1_1, sul1_5, sul2_2, tet(A)_6, tet(B)_1, tet(B)_2, aac(3)-IIa_1, aac(6')-Ib-cr_1, aadA2_1, ant(3'')-Ia_1, aph(3')-Ia_1, blaOXA-10_1, blaOXA-1_1, blaSHV-106_1, blaSHV-145_1, blaSHV-27_1, cmlA1_1, dfrA12_8, dfrA7_5, fosA6_1, fosA_6, qnrB1_1, NA, aph(3')-IIb_2, blaOXA-485_1, blaOXA-486_1, blaPAO_2, blaPAO_4, catB7_1, fosA_4, blaSED1_1 

Region: Aeromonas_spp&Pseudomonas aeruginosa 
Elements: NA 

Region: Citrobacter freundii 
Elements: c("blaCMY-98_1", "qnrB69_1") 

Region: Enterobacter cloacae 
Elements: c("blaCMH-3_1", "fosA_7", "oqxA_1", "oqxB_1") 

Region: Escherichia coli 
Elements: c("aadA5_1", "aph(3'')-Ib_2", "aph(3'')-Ib_5", "aph(3')-IIa_2", "aph(6)-Id_1", "blaACT-6_1", "blaCMY-2_1", "blaCTX-M-130_1", "blaCTX-M-15_1", "blaCTX-M-27_1", "blaCTX-M-55_1", "blaCTX-M-8_1", "blaTEM-141_1", "blaTEM-1B_1", "dfrA14_5", "dfrA17_1", "floR_2", "fosA_1", "mdf(A)_1", "mph(A)_2", "oqxA_1", "oqxB_1", "qnrS1_1", "sul1_5", "sul2_2", "tet(A)_6", "tet(B)_1", "tet(B)_2") 

Region: Klebsiella pneumoniae 
Elements: c("aac(3)-IIa_1", "aac(6')-Ib-cr_1", "aadA2_1", "ant(3'')-Ia_1", "aph(3'')-Ib_5", "aph(3')-Ia_1", "aph(6)-Id_1", "blaCTX-M-15_1", "blaOXA-10_1", "blaOXA-1_1", "blaSHV-106_1", "blaSHV-145_1", "blaSHV-27_1", "blaTEM-1B_1", "cmlA1_1", "dfrA12_8", "dfrA14_5", "dfrA7_5", "fosA6_1", "fosA_6", "mph(A)_2", "oqxA_1", "oqxB_1", "qnrB1_1", "sul1_5", "sul2_2", "tet(A)_6") 

Region: Only_Aeromonas_spp 
Elements: ampH_1, ampH_2, blaCMY-8b_1, cphA1_1, cphA2_6, blaSED1_1 

Region: Only_Citrobacter freundii 
Elements: c("blaCMY-98_1", "qnrB69_1") 

Region: Only_Enterobacter cloacae 
Elements: c("blaCMH-3_1", "fosA_7", "oqxA_1", "oqxB_1") 

Region: Only_Escherichia coli 
Elements: c("aadA5_1", "aph(3'')-Ib_2", "aph(3'')-Ib_5", "aph(3')-IIa_2", "aph(6)-Id_1", "blaACT-6_1", "blaCMY-2_1", "blaCTX-M-130_1", "blaCTX-M-15_1", "blaCTX-M-27_1", "blaCTX-M-55_1", "blaCTX-M-8_1", "blaTEM-141_1", "blaTEM-1B_1", "dfrA14_5", "dfrA17_1", "floR_2", "fosA_1", "mdf(A)_1", "mph(A)_2", "oqxA_1", "oqxB_1", "qnrS1_1", "sul1_5", "sul2_2", "tet(A)_6", "tet(B)_1", "tet(B)_2") 

Region: Only_Klebsiella pneumoniae 
Elements: c("aac(3)-IIa_1", "aac(6')-Ib-cr_1", "aadA2_1", "ant(3'')-Ia_1", "aph(3'')-Ib_5", "aph(3')-Ia_1", "aph(6)-Id_1", "blaCTX-M-15_1", "blaOXA-10_1", "blaOXA-1_1", "blaSHV-106_1", "blaSHV-145_1", "blaSHV-27_1", "blaTEM-1B_1", "cmlA1_1", "dfrA12_8", "dfrA14_5", "dfrA7_5", "fosA6_1", "fosA_6", "mph(A)_2", "oqxA_1", "oqxB_1", "qnrB1_1", "sul1_5", "sul2_2", "tet(A)_6") 

Region: Only_Pseudomonas putida 
Elements: c("aph(3')-IIb_2", "blaOXA-485_1", "blaOXA-486_1", "blaPAO_2", "blaPAO_4", "catB7_1", "fosA_4") 

Region: Pseudomonas aeruginosa 
Elements: NA 

Region: Pseudomonas putida 
Elements: c("aph(3')-IIb_2", "blaOXA-485_1", "blaOXA-486_1", "blaPAO_2", "blaPAO_4", "catB7_1", "fosA_4") 
...
sample_2_species_df %>% group_by( Specie_Name )  %>% summarize( numSamples = n() )
...
#e.coli.st = mlst_tsv_df %>% 
e.coli.st = resultV2 %>%                     # resultsV2 has dup removed
  group_by( PubMLST_scheme_name ) %>%
  filter( PubMLST_scheme_name == "ecoli" ) %>%
  group_by( SeqType ) %>%
  summarize( stCount = n() )
  # %>% summarize( numSamples = n() )

sum( e.coli.st$stCount )  # 21 E.coli
...
resultV2 %>%
  group_by( PubMLST_scheme_name ) %>%
  summarize( speciesCount = n() )      # 21 E.coli



resultV2 %>%                     # resultsV2 has dup removed
  group_by( PubMLST_scheme_name ) %>%
  filter( PubMLST_scheme_name != "kpneumoniae" ) %>%
  group_by( SeqType ) %>%
  select( Specie_Name, PubMLST_scheme_name, key, SeqType ) 

# paeruginosa
resultV2 %>%                     # resultsV2 has dup removed
  group_by( PubMLST_scheme_name ) %>%
  filter( PubMLST_scheme_name == "paeruginosa" ) %>%
  group_by( SeqType ) %>%
  select( Specie_Name, PubMLST_scheme_name, key, SeqType ) 
...
resultV2 %>%                     
  select( Specie_Name, PubMLST_scheme_name, key, SeqType ) %>%
  #xx mutate( STinThisSpecie = )
  pivot_wider( names_from=SeqType, values_from=Specie_Name )
...
for ( specie in SpecieList ) {
  STinSpecie[specie] = ( resultV2 %>% filter( Specie_Name == specie ) %>% pull( SeqType ) )
}


# q: how to append to a list?



# create a list of ST in each specie
for ( specie in SpecieList ) {
   ST_list_for_this_specie = ( resultV2 %>% filter( Specie_Name == specie ) %>% pull( SeqType ) %>% unique() %>% sort() )
   #print( specie )
   #print( ST_list_for_this_specie )
   STinSpecie[[specie]] = ST_list_for_this_specie
}

str( STinSpecie )
#xx colnames( STinSpecie )


# convert STinSpecie into a 2D table

for ( i in 1:length( STinSpecie ) ) {
  STinSpecie[[i]] = paste( STinSpecie[[i]], collapse="," )
  print(  names( STinSpecie )[i] )
  print( STinSpecie[[i]] )
}

STinSpecie

# make table out of above  (maybe in Excel)
...
```{r scratchpad}
# number of samples shown in pie chart
3+1+1+21+3+8+2+5

resultV2 %>% filter( RESIST_NUM_FOUND <  1 )   # 12 rows
resultV2 %>% filter( RESIST_NUM_FOUND >= 1 )   # 32 rows   total 44 samples.

32/44  # 72.7% of samples have resistance genes


resultV2 %>% filter( `source` != "SHRIMP" ) # 31 rows
resultV2 %>% filter( `source` == "SHRIMP" ) # 13 rows

7/13  # 53.8% of shrimp samples have resistance genes
20/31  # 64.5% of non-shrimp samples have resistance genes
...
# MOB_recon mge report analysis

(from OneNotes | phd_prj | Shrimp | MOB-suite, MGEanalysis )

>> do this, check procedure with Niko/Jay ++

	so, procedure should be (get confirmation):
		1. run mob_recon using assembled seq generated by unicycler
		2. Check mob_recon report , 
			i. filter for "plasmid" (drop chromosome).
			ii. get primary_cluster_id and secondary_cluster_id
			iii. these would be list of legit plasmid sequences (containing known replicon, relaxase).
			iv. (but the blue highligh portion above, is discarded, doesn't save to a plasmid_NNN.fasta file?)
			v. run abricate on the plasmid listed in (iii).  
				1) If ran abricate on all plasmid_NNN.fasta, then use R to join and filter out those what were not in the mob_recon report.
				
	Potential stat for Shrimp paper, some easier to generate than other.
		- num of ARG found to be in plasmid
		- num of isolate found to have plasmid (in the mob_recon report, was not 100%)
		- join with prior (resultV2 df) and identify, per sample, which has plasmid/MGE and/or which ARG is plasmid-bound.


## first define join key

MOBrecon_mge_result_tsv sample_id eg:

AA_CKDN230030154-1A_HGKHYDSX7_L2
2^ ^^^^3^^^^^^^^ 4^ ^5^^^^^^^ ^6

resfinderOnPlasmid_lng_tsv  #FILE  eg:
AA_CKDN230030154-1A_HGKHYDSX7_L2.MOBre/plasmid_AA474.fasta
2^ ^^^^3^^^^^^^^ 4^ ^5^^^^^^^ ^6 ^7^^^         ^8^^^
AP_CKDN230030169-1A_HGKHYDSX7_L2.MOBre/plasmid_novel_659910db89b47e9b96450005af46f7bd.fasta   # these novel plasmid never show up in the mob_recon mge report... maybe can ignore them



```{r mge_plasmid_join_key}

# will be joining these, or subset of them
#View( MOBrecon_mge_result_tsv )
#View( resfinderOnPlasmid_lng_tsv )

MOBrecon_mge_result_df = MOBrecon_mge_result_tsv %>%
  #                                                2                 3                    4                  5                      6              
  mutate( sample_name     = str_match( sample_id, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)"  )[,2] ) %>%
  mutate( Hnum            = str_match( sample_id, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)"  )[,5] ) %>%
  mutate( Lnum            = str_match( sample_id, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)"  )[,6] )



# split contid_id into contid_name and contid_descript , name (likely just number) is what abricate report under "SEQUENCE", so can do matching...
# eg from AA_CKDN230030154-1A_HGKHYDSX7_L2.fasta
# >55 length=1270 depth=7.89x 
#  ^2 ^3^^^^^^^^^^^^^^^^^^^^^
# the contig id stripped the initial > marker of fasta name/description line
MOBrecon_mge_result_df = MOBrecon_mge_result_df %>%
  #                                                2                 
  mutate(  contig_name     = str_match( contig_id, "([:alnum:]+)[\\s]"  )[,2] )  %>%
  mutate(  contig_descrip  = str_match( contig_id, "([:alnum:]+)[\\s]([[:alnum:]=\\s]+)"  )[,3] ) # %>%
  #mutate( contig_name     = str_match( contig_id, "([:alpha:]+)[\\s]([a-zA-Z0-9= ]+)"  )[,2] ) # %>%
#  [:alnum:]      match alpha numeric, [:digit:] match digit only, [:space:] match space only
# [[:alnum:]=\\s] match alpha numeric, equal sign, whitespace \\s 

MOBrecon_mge_result_lng = MOBrecon_mge_result_df %>%
  pivot_longer( cols=c( primary_cluster_id, secondary_cluster_id ), names_to="cluster_type", values_to="cluster_id" ) %>%
  drop_na(cluster_id) 

MOBrecon_mge_result_lng = MOBrecon_mge_result_lng %>%
  mutate( jKey            = str_c( sample_name, "_H", Hnum, "_L", Lnum, "_", cluster_id,  sep="" ) )

# View( MOBrecon_mge_result_lng )  # 142 rows,  jKey NOT unique!!
mge_cluster_uniq = MOBrecon_mge_result_lng %>% select( jKey, sample_id, cluster_id, molecule_type ) %>% unique() # %>% length()  # 46...
  
# if select these, wont be unique, likely cuz secondary_cluster_id has diff mge id : mge_id, mge_acs, mge_type, mge_subtype  

View( mge_cluster_uniq )

resfinderOnPlasmid_lng_df = resfinderOnPlasmid_lng_tsv %>%
  #                                                2                 3                    4                  5                      6              7                8
  mutate( sample_name     = str_match( file_name, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)\\.(MOBre)/plasmid_([A-Z0-9a-z_]+)\\.fasta"  )[,2] ) %>%
  mutate( Hnum            = str_match( file_name, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)\\.(MOBre)/plasmid_([A-Z0-9a-z_]+)\\.fasta"  )[,5] ) %>%
  mutate( Lnum            = str_match( file_name, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)\\.(MOBre)/plasmid_([A-Z0-9a-z_]+)\\.fasta"  )[,6] ) %>%
  mutate( plasmid_id      = str_match( file_name, "([:alpha:]+)[\\-_](CKDN[:digit:]+)[\\-]([:alnum:]+)[\\-_]H([:alnum:]+)DSX7[\\-_]L([:alnum:]+)\\.(MOBre)/plasmid_([A-Z0-9a-z_]+)\\.fasta"  )[,8] ) %>%
  mutate( jKey            = str_c( sample_name, "_H", Hnum, "_L", Lnum, "_", plasmid_id,  sep="" ) )
  
  

View( resfinderOnPlasmid_lng_df )  # 120 rows, not unique due to multiple ARG per plasmid possible, even likely.
```


```{r tmp_mge_plasmid_check, eval=F}

# this was for dev, need to change to use joined tables

#plasmid_list = MOBrecon_mge_result_tsv %>% filter( molecule_type == "plasmid" ) %>% pull( primary_cluster_id, secondary_cluster_id ) %>% unique()
# problem with this is that lost sample_id info

plasmid_list = MOBrecon_mge_result_tsv %>% filter( molecule_type == "plasmid" ) %>% select( sample_id, primary_cluster_id, secondary_cluster_id ) %>% unique()
# yeah, generate some key and do join...
plasmid_list
...
#//plasmid_in_mge_result = MOBrecon_mge_result_tsv %>% filter( molecule_type == "plasmid" ) %>% select( sampleIdJoinKey, primary_cluster_id, secondary_cluster_id ) %>% unique() 
  ##%>%   pivot_longer( cols=c( primary_cluster_id, secondary_cluster_id ), names_to="cluster_type", values_to="cluster_id" )

                

#//plasmid_in_mge_result %>% select( sampleIdJoinKey ) %>% unique()   # 21 samples

#mgeResult_arg = right_join( MOBrecon_mge_result_lng, resfinderOnPlasmid_lng_df, by="jKey" )

#mgeResult_arg = right_join( resfinderOnPlasmid_lng_df, MOBrecon_mge_result_lng, by="jKey" )
mgeResult_arg = left_join( mge_cluster_uniq, resfinderOnPlasmid_lng_df, by="jKey" )

# right join has 504 rows, not 1:1
# left  join has 126 rows

View( mgeResult_arg )  # 126 rows.  


mgeResult4discuss = mgeResult_arg %>% select( sample_id, sample_name, cluster_id, molecule_type, GENE, PRODUCT, ACCESSION, `%COVERAGE`, `%IDENTITY` ) 

mgeResult4discuss %>% filter( !is.na(ACCESSION) ) # 99 rows, ie 99 arg within plasmid.

View( mgeResult4discuss )
writexl::write_xlsx( mgeResult4discuss, "TMP/mgeResult4discuss.DRAFT.XLSX" )   # uploaded and converted to gSheet, might annotate there for notes for discussion on paper.
# https://docs.google.com/spreadsheets/d/1o_1AH5gUkLyLRPWziMKVw8rWhfg1sqHyuhAjCTGSx3o/edit?gid=547027573#gid=547027573 

```


```{r quickAssest_NumSampleWithEsblFromMge}


mge_join_sampleInfo = full_join(
  #x  = mgeResult4discuss,
  x  = mgeResult_arg,
  y  = sample_metadata_withDupInfo,
  by = "sample_name"
)

View( mge_join_sampleInfo )

mge_sampleWithEsbl = 
mge_join_sampleInfo %>%
  filter( is.na( DuplicateInfo ) ) %>%
  filter( grepl("blaCTX-M|blaTEM|blaSHV|blaOXA", GENE) )

# 16 rows have ESBL on plasmid.

mge_sampleWithEsbl %>% pull( jKey      ) %>% unique() # %>% length()  # 8 unique entries (some sample could have multiple plasmid_id)
mge_sampleWithEsbl %>% pull( sample_id ) %>% unique() # %>% length()  # 7 unique entries, out of 44 samples used  # so this is proper sample count with ESBL.

# what about ESBL not from plasmid?


View( mge_sampleWithEsbl )


resfinderLabeledNoDup_join_sampleInfo =
full_join( 
  x  = resfinder_labeled_noDup,
  y  = sample_metadata_withDupInfo,
  by = "sample_name"
) 
# full_join (originally used this) has 185 rows
# right_join also has 185 rows, 

#// back spot check, 
resfinderLabeledNoDup_join_sampleInfo %>% select( sample_name ) %>% unique() %>% nrow()  # 50 rows with right or full join.


View( resfinderLabeledNoDup_join_sampleInfo )
#//  "D_|E_|L_|AC_|AE_|AK_|AL_|AN_|AW_|AZ_|BC_|BD_"
#//   n  y  NA NA  NA  NA  NA  AN  NA  NA  NA  NA
#//   NA are rows that is in sampleInfo, but NA for resfinder.  so samples that has no resistance gene, which is fine, cuz comparing resistance found vs not found.
#//   supplementary table is for resistance found, not a complete list of samples used
#//   overall, sequence submission should be based in resultV2 (which is from SampleInfo), not the Supplementary table.  




sampleWithEsbl =  # these dont know if from plasmid yet
resfinderLabeledNoDup_join_sampleInfo %>%
    filter( grepl("blaCTX-M|blaTEM|blaSHV|blaOXA", gene) )
...
sampleWithEsbl %>% pull( sample_name ) %>% unique() # %>% length()  # 18 unique entries, out of 44 samples used
sampleWithEsbl %>% pull( key.x       ) %>% unique() %>% length()  # 18 unique entries, out of 44 samples used

```

Jay called and wanted to state in paper:

Number of samples with ESBL that are from chromosome vs plasmid.  
Think the info below (quickly calculated from 1-2 chunks above) can answer that.

1.
Number of samples with bla* genes: 18   (don't know if they are from plasmid at this count).

2.
Number of samples with plasmid containing bla* genes : 7  (out of total of 44 samples).


so, i think we can say 11 are ESBL not in plasmid, 7 are ESBL in plasmid, and (44-18 = 26) does not have ESBL.
(these are across all organism, as 44 include non E.coli)


1. "AA" "AB" "AI" "AM" "AP" "AR" "AV" "AX" "AY" "A" "C" "E" "H" "J" "K" "S" "T" "Y" 
2.              AI   AM         AR          AX         A        E                          Y



```{r describeFindingOfAbricateOnPlasmid}
View( mgeResult4discuss )
# writexl::write_xlsx( mgeResult4discuss, "TMP/mgeResult4discuss.DRAFT.XLSX" )  # 

mgeResult4discuss %>% pull( sample_name ) %>% unique() %>% length()  # 16 unique entries, out of 44 samples used
mgeResult4discuss %>% pull( sample_name ) %>% unique() #  [1] "AA" "AF" NA   "AI" "AM" "AP" "AR" "AS" "AX" "A"  "B"  "E"  "P"  "Q"  "R"  "Y" 
mgeResult4discuss %>% pull( GENE ) %>% unique() %>% length()  # 30 genes
mgeResult4discuss %>% pull( GENE ) %>% unique() 
argFromAbricateOnPlasmid = mgeResult4discuss %>% select( GENE ) %>% unique() 
# genes includes "tet(B)_1"        "blaCTX-M-130_1" 

View( argFromAbricateOnPlasmid )
argFromAbricateOnPlasmid %>% filter( grepl("bla|tet", GENE) )
argFromAbricateOnPlasmid %>% filter( grepl("bla", GENE) )



# the bla* genes are ESBL, tet is not.
# 9 bla* genes:
# blaTEM, blaCTX-M, blaOXA, blaCMY
...
check for NA in file_name (ie right DF) from above, some 26 of them!
AF_HGKHY_L2_AI077   AF_CKDN230030159-1A_HGKHYDSX7_L2  AI077  plasmid 
AI_HGKHY_L2_AI612   AI_CKDN230030162-1A_HGKHYDSX7_L2  AI612  plasmid

Apparently, the MOB_recon MGE report can have cluster_id (maybe secondary only?) where there is no plasmid_${CLUSTERID}.fasta file, thus NA in filename on right DF.


# Niko's method of determining if ARG is in MGE/plasmid

	Niko's process via bmail.
	essentially look for overlap in contig region where AMR was found and where plasmid is found.
	can probably write another Rmd chunk to do this and see if they correlate.



```{r mge_plasmid_NikoMethod}

# View( MOBrecon_mge_result_df )
mge_location = MOBrecon_mge_result_df %>% 
  filter( molecule_type != "chromosome" ) %>%      # 81 rows
  #filter( molecule_type == "plasmid" ) %>%        # 81 rows, so all mge are reported as plasmid
  select( sample_id, sample_name, primary_cluster_id, secondary_cluster_id, contig_id, contig_name, contig_descrip, contig_start, contig_end, length, sstrand )

# View( mge_location )

#View( resfinder_labeled_noDup )

resfinder_location = resfinder_labeled_noDup %>% 
  select( file_name.x, sample_name, START, END, STRAND, SEQUENCE, STRAND, gene, PRODUCT, ACCESSION )


mgeLocPlusARG = full_join( mge_location, resfinder_location,
                           relationship = "many-to-many",
                           by=c("sample_name"="sample_name") )  # M:M left_join = 524 rows, full_join = 579 

#View( mgeLocPlusARG )  # 524 rows, M:M join likely many stray records


mgeContigSame = mgeLocPlusARG %>%
  mutate( sameContig = if_else( ( contig_name == SEQUENCE ), TRUE, FALSE )
          )

View( mgeContigSame ) # only 11 rows match, then if compare +/- strand even fewer match ++ 



mgeLocPlusARG = mgeLocPlusARG %>%
  mutate( argInsideMGE = if_else( ( ( as.numeric(contig_start) <= START ) & 
                                    ( as.numeric(contig_end)   >= END   ) 
                                  ), TRUE, FALSE 
                                  )
          )   


argInsideMGE = mgeLocPlusARG %>% 
  select( - file_name.x, - length ) %>%
  filter( argInsideMGE == TRUE )  # 44 rows     and sstrand vs STRAND does not always match

View( argInsideMGE )  # 44 rows, still 44 rows if use full_join

writexl::write_xlsx( argInsideMGE, "TMP/argInsideMGE.NikoMethod.DRAFT.XLSX" )
...
summary( resfinder_labeled_noDup$START  )
summary( resfinder_labeled_noDup$END  )

summary( as.numeric( MOBrecon_mge_result_df$contig_start ) )  # 1_701_480  max
summary( as.numeric( MOBrecon_mge_result_df$contig_end   ) )  # 1_702_706 
summary( as.numeric( MOBrecon_mge_result_df$mge_start    ) )  # 1_698
summary( as.numeric( MOBrecon_mge_result_df$mge_end      ) )  # 6_172
summary( as.numeric( MOBrecon_mge_result_df$contig_id    ) )  #   lot of stuff in here, cuz it is the whole header line of the contig id (abricate only display the first word)
summary( as.numeric( mge_location$contig_end ) )              # 84_064  ## plasmid only, 
summary( as.numeric( mge_location$length ) )                  # 4_475 

max( resfinder_labeled_noDup$START )   # 779_274  
max( resfinder_labeled_noDup$END   )   # 780_062  # much earlier than contig_end ... ++
max( resfinder_labeled_noDup$SEQUENCE ) # 535  ## what is this? Abricate resfinder... 

## gonna use my method, run abricate on plasmid generated by mob_recon.
...
this was requested by reviewer
wanting to see %identity of the ARG.

```{r generateSupplementTableOnResfinder }
#View( resfinder_labeled )           # 225 rows, match Abricate_resfinder_combined.tsv 
View( resfinder_labeled_noDup )      #  167 rows after dup removal so this is right df after all
#View( resfind_sum_pct_df )          # this is the wide format.  not for here.

#View( sample_metadata_withDupInfo ) # dont need this, dup already removed.

  ##x  = resfinder_labeled_noDup,

NOT_this_xx_supplementTabRaw = full_join(
  x  = resfinder_labeled_noDup, 
  y  = sample_metadata_withDupInfo, 
  by = "sample_name"
  ) 

View( NOT_this_xx_supplementTabRaw )  # 185 rows, 5 are marked with "Dup" 

# select these columns: #FILE   SEQUENCE        START   END     STRAND  GENE    COVERAGE        COVERAGE_MAP    GAPS    %COVERAGE       %IDENTITY       DATABASE        ACCESSION       PRODUCT RESISTANCE 

supplementTab = 
resfinder_labeled_noDup %>% 
  select( file_name.y, SEQUENCE, START, END, STRAND, gene, `COVERAGE`, COVERAGE_MAP, GAPS, `%COVERAGE`, `%IDENTITY`, DATABASE, ACCESSION, PRODUCT, RESISTANCE ) %>%
  rename( `#FILE` = file_name.y,
           GENE   = gene, 
        )  

View( supplementTab ) # 167 rows

write_tsv( supplementTab, "result/notGood_supplementTable.tsv" )
# no, above look doctored.  Abricate has 100.00 but R writes 100 .  R also have NA added in places.
# also, there are some extra records that should not be there, cuz didn't filter by Hnum "GKHY", there were some L1 that wasn't used.
# rc3 below is good.

# gonna use Unix egrep.

View( sample_metadata_withDupInfo ) # dont need this, dup already removed.

# Dups are AF P Q AG AT 5 of em.  I removed elsewhere cuz dont have the sequence for it...

nonDupSampleList = sample_metadata_withDupInfo %>% 
  filter( is.na( DuplicateInfo ) )  %>%
  select( sample_name )

  
View( nonDupSampleList )  # 45 rows (cuz include I here), 

nonDupFilePatternList = sample_metadata_withDupInfo %>% 
  #mutate( filenamePattern = paste0( Hnum, "DSX7_L", Lnum) ) %>%
  #select( filenamePattern )
  select( Hnum )

nonDupFilePatternList %>% unique
# just one, GKHY
#           GKHYDSX7_L   # filename 
#               DSX7_L   # invariant I found that I stripped

write_tsv( nonDupSampleList, "result/nonDupSampleList.tsv"  )
#write.table( nonDupSampleList, "result/nonDupSampleList.tsv", quote=FALSE, row.names=FALSE, col.names=FALSE, sep="|" )     # sep="\t"  # very weird, the file is right, but after `| xargs`, the is only 2 letters!


# cat nonDupSampleList.tsv   | xargs | sed 's/\ /\|/g'
# sample_name|AA|AB|AC|AD|AE|AH|AI|AJ|AK|AL|AM|AN|AO|AP|AQ|AR|AS|AU|AV|AW|AX|AY|AZ|A|BA|BB|BC|BD|B|C|D|E|F|G|H|I|J|K|L|R|S|T|W|Y|Z

# cat nonDupSampleList.tsv  | xargs | sed 's/\ /_|^/g'
# sample_name_|^AA_|^AB_|^AC_|^AD_|^AE_|^AH_|^AI_|^AJ_|^AK_|^AL_|^AM_|^AN_|^AO_|^AP_|^AQ_|^AR_|^AS_|^AU_|^AV_|^AW_|^AX_|^AY_|^AZ_|^A_|^BA_|^BB_|^BC_|^BD_|^B_|^C_|^D_|^E_|^F_|^G_|^H_|^I_|^J_|^K_|^L_|^R_|^S_|^T_|^W_|^Y_|^Z


# cat Abricate_resfinder_combined.csv  | egrep --color GKHYDSX7_L2 | egrep --color  '^AA_|^AB_|^AC_|^AD_|^AE_|^AH_|^AI_|^AJ_|^AK_|^AL_|^AM_|^AN_|^AO_|^AP_|^AQ_|^AR_|^AS_|^AU_|^AV_|^AW_|^AX_|^AY_|^AZ_|^A_|^BA_|^BB_|^BC_|^BD_|^B_|^C_|^D_|^E_|^F_|^G_|^H_|^I_|^J_|^K_|^L_|^R_|^S_|^T_|^W_|^Y_|^Z_' > TMP_sup1_rc3.tsv 

# down to 153 rows, 
#// hmmm.... cat Abricate_resfinder_combined.csv | egrep "D_|E_|L_|AC_|AE_|AK_|AL_|AN_|AW_|AZ_|BC_|BD_"
#// has AD_ E_ only 


## now.  cross checked with the incremental improvement, as well as the table R would have generated.  
# manually add header line.  153 rows, including header line


# supplementTable.tsv seems to ahve 33 uniq filenames...
# supplementTable does not have: AC AE  AK AL AN AW AZ BC BD D E L 
# per vimdiff supplementTable.filename.UNIQ.cut1-35.TMP resultV2_filename_list.SORT-U.cut1-35.TMP
# diff ... > supplementTable_vs_resultV2_diff.TMP 

# cuz they were grepped out.  
resultV2 %>% select( Filename ) %>% unique()  %>% nrow()  # 44 rows

resultV2_filename_list = resultV2 %>% select( Filename ) # 
write_tsv( resultV2_filename_list, "TMP/resultV2_filename_list.tsv"  )
...
more added now, better to cat Rmd | grep View
...
# write_tsv( resultV2, "TMP/resultV2.tsv" )   # this is a big data-warehouse style table, sometime Unix is quicker to parse :D
# supplementTable.tsv seems to ahve 33 uniq filenames...
