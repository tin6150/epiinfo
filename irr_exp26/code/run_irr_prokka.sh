...
## epiinfo Irrigation Expposure 26  LeBoa

## adapted from guatemala/run_guat_prokka.sh (Practicum prj)

## be careful with the path each of the function cd into  !!

#### NOTE: gff in certain filename format.  (strict taxa name in (phylip) limited to 10 chars)

## run as: sbatch run_irr_prokka.sh

#SBATCH --job-name=irr_prokka
...
# needed only when fasta file is inside a dir foo.fasta/assembly.fasta 
# which is not the case for irr_exp26 seq
######################################################################
run_prep4prokka () {
	# list of commands
...
backfil_A30 () {
	 singularity exec /clusterfs/vector/home/groups/software/sl-7.x86_64/modules/prokka/1.14.5/prokka.sif prokka --help
	#~~ singularity exec /clusterfs/vector/home/groups/software/sl-7.x86_64/modules/prokka/1.14.5/prokka.sif prokka --outdir PROKKA_3 --prefix h2_A30 --compliant --force A30_CKDN220053900-1A_HK7HVDSX5_L1.fasta 
...
run_roary_find_dups () {
	cd ~/gs/guatemala_amr/assembled-sequences_sn/Fasta4Prokka/PROKKA_3
	#cd Fasta4Prokka
	#cd ./PROKKA         #  gff to 9CHAR
	cd diff_A1_A5  # hmm... ran roary to check for this diff already...
	singularity exec /clusterfs/vector/home/groups/software/sl-7.x86_64/modules/phylotool/u20.04/phylotool.sif roary -e -n -mafft -p $Thread *.gff
	cd ..
	# dropped, moving to use /opt/cd-hit/cd-hit-v4.8.1-2019-0228/cd-hit-auxtools/cd-hit-dup
	# resuming, but in a new file, run_guat_prokka_chkDup.sh, to cross check if result points to dup
